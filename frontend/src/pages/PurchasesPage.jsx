...
export default function PurchasesPage() {
  ...
  const poList = useAsync(
    () =>
      purchaseOrderService.list({
        ...
        fromDate: poFromDate || undefined,
        toDate: poToDate || undefined,
        page: poPage,
        limit: LIMIT,
      }),
  ...
  function handlePoSearchSubmit(event) {
    event.preventDefault();
    setPoSearch(poSearchInput.trim());
    setPoPage(1);
  }
  ...
  function handlePoStatusFilter(event) {
    setPoStatus(event.target.value);
    setPoPage(1);
  }

  function handlePoFromDateFilter(event) {
    setPoFromDate(event.target.value);
    setPoPage(1);
  }

  function handlePoToDateFilter(event) {
    setPoToDate(event.target.value);
    setPoPage(1);
  }
  ...
  const openPoDetail = useCallback((id) => setPoDetailId(id), []);
    ...
    <div className="purchases-page">
      ...
      <div className="purchase-tabs" role="tablist">
        <button
          type="button"
          role="tab"
          aria-selected={tab === 'purchase-orders'}
          className={`purchase-tab ${tab === 'purchase-orders' ? 'is-active' : ''}`}
        ...
        <button
          ...
          className={`purchase-tab ${tab === 'history' ? 'is-active' : ''}`}
      ...
      {tab === 'purchase-orders' ? (
        <>
          <div className="toolbar">
            <form className="toolbar-search" onSubmit={handlePoSearchSubmit} role="search">
              <input
                type="search"
                value={poSearchInput}
                onChange={(event) => setPoSearchInput(event.target.value)}
              ...
              <button type="button" className="btn btn-outline" onClick={handlePoReset}>
                Reset
              </button>
            </form>
            ...
            <div className="toolbar-actions">
              <label className="toolbar-select">
                ...
                <select value={poStatus} onChange={handlePoStatusFilter}>
              ...
              <label className="toolbar-select">
                ...
                <input
                  type="date"
                  value={poFromDate}
                  onChange={handlePoFromDateFilter}
          ...
          {poList.loading && !poList.data ? (
            <PageLoader label="Loading purchase orders…" />
          ) : null}

          {poList.error && !poList.data ? (
            <ErrorState
              title="Purchase orders unavailable"
              message={poList.error.message}
              status={poList.error.status}
              onRetry={() => poList.refetch()}
            />
          ) : null}

          {poList.data && poItems.length === 0 ? (
            <EmptyState
              title={poHasFilters ? 'No purchase orders match your filters' : 'No purchase orders yet'}
              description={
                poHasFilters
                  ? 'Try changing the search or clearing the filters.'
                  : 'Create a purchase order to order goods from a supplier. Stock only changes once goods are received.'
              }
            />
          ) : null}
            ...
            <div className="table-card">
              <div className="table-tools">
                <p className="table-count">
                  {poPagination?.total ?? 0} purchase order{poPagination?.total === 1 ? '' : 's'}
                </p>
                {poList.loading ? <span className="table-refreshing">Refreshing…</span> : null}
              </div>
              ...
              <div className="table-scroll">
                <table className="data-table">
                  ...
                  <tbody>
                    {poItems.map((order) => {
                      const outstanding = Number(order.outstanding_total) || 0;
                      return (
                        <tr key={order.id}>
                          <td className="cell-main cell-code">{order.po_number}</td>
                          <td>{formatDateOnly(order.order_date)}</td>
                          <td>{order.supplier_name || '—'}</td>
                          <td>{formatDateOnly(order.expected_delivery_date)}</td>
                          <td className="num">{formatQuantity(order.received_total)}</td>
                          <td className="num">
                            {outstanding > 0 ? (
                          ...
                          <td className="actions-col">
                            <div className="table-actions">
                              <ActionButton action="view" onClick={() => openPoDetail(order.id)} />
                            </div>
                          </td>
                        </tr>
                      );
              ...
              <Pagination
                page={poPagination?.page ?? 1}
                totalPages={poPagination?.totalPages ?? 1}
                onChange={setPoPage}
              />
            </div>
          ) : null}
          ...
          {createPoOpen ? (
            <PurchaseOrderFormModal
              ...
              onSaved={(order) => {
                ...
                refreshAll();
                showToast(`Purchase order ${order?.po_number || ''} created.`, 'success');
                setPoDetailId(order?.id ?? null);
              }}
            />
          ) : null}
        </>
              ...
              {detail ? (
                <div className="purchase-detail">
                  <div className="summary-grid" style={{ marginBottom: 16 }}>
                    ...
                    <div className="summary-item">
                      <span className="summary-label">Purchase Order</span>
